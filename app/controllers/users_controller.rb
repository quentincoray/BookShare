class UsersController < ApplicationController
  ...
  def show
    ...
    @commented_orders = Order.all.where.not(review: nil).where(user: current_user).order("updated_at DESC")
    @uncommented_orders = Order.all.where(review: nil, order_status: "paid").where(user: current_user)
  ...
  def user_params
    params.require(:user).permit(:name, :description, :password, :password_confirmation, :photo)
  end
end
